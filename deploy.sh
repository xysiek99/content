...
if ! "$MAIN_DIRECTORY/$CHECK_ANSIBLE_SECRETS_SCRIPT" "$MAIN_DIRECTORY/$ANSIBLE_DIRECTORY/$ANSIBLE_VAULT_PWD_FILE" "$MAIN_DIRECTORY/$ANSIBLE_DIRECTORY/$ANSIBLE_PHP_IMG_DEPLOY_SECRETS_FILE"; then
  exit 1
fi

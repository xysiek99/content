...
function myFunction(val, keyvalue) {
  if(val.toLowerCase().includes(keyvalue.toLowerCase()) !== false){
...
function HTMLtoPDF() {
    ...
    //doc.addHTML($('#toPDF'), 15, 15, function() {
    //doc.save('div.pdf');
  //});
  doc.text(username, 20, 30);
  doc.text(", ukonczyles quiz.", 90, 30);
  doc.text("Twój wynik to: ", 20, 70);
  doc.text(score, 120, 70);
  //doc.text(20, 30, "This is client-side Javascript, pumping out a PDF.");
  //doc.addPage();
  //doc.text(20, 20, "Do you like that?");

  // Save the PDF
  doc.save("Wynik quizu.pdf");
}
